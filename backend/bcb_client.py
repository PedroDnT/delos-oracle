...
class BCBClient:
    ...
    def _validate_value(
        self, rate_type: RateType, scaled_value: int, raw_value: float
    ) -> None:
        """
    ...
    def _process_response(
        self, data: List[Dict], rate_type: RateType
    ) -> List[RateData]:
        """
    ...
    async def fetch_history(
        self, rate_type: RateType, count: int = 10
    ) -> List[RateData]:
        """
    ...
    async def fetch_date_range(
        self, rate_type: RateType, start_date: datetime, end_date: datetime
    ) -> List[RateData]:
        """
    ...
    async def fetch_all_latest_parallel(
        self, rate_types: Optional[List[RateType]] = None
        ...
        async def fetch_with_error_handling(
            rate_type: RateType,
        ) -> Tuple[RateType, Optional[RateData]]:
        ...
        return {rate_type: data for rate_type, data in results if data is not None}
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        async with BCBClient() as client:
            ...
            print(
                f"{'Rate':<6} | {'Raw Value':>12} | {'Chainlink (8 dec)':>18} | {'Basis Pts':>10} | {'Date':>12} | Source"
            )
            print("-" * 85)
            ...
            for rate_type, config in RATE_CONFIGS.items():
                ...
                print(
                    f"  Min/Max: {config.min_value / CHAINLINK_PRECISION} / {config.max_value / CHAINLINK_PRECISION}"
                )
