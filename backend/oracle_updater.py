...
class OracleUpdater:
    ...
    async def check_needs_update(self, rate_data: RateData) -> Tuple[bool, str]:
        ...
        if current["real_world_date"] > rate_data.real_world_date:
            return (
                False,
                f"bcb_data_older:{rate_data.real_world_date}<{current['real_world_date']}",
            )

        return (
            True,
            f"new_date:{current['real_world_date']}->{rate_data.real_world_date}",
        )
    ...
    async def _send_single_update(self, rate_data: RateData) -> UpdateResult:
        ...
        try:
            logger.info(
                f"Updating {rate_data.rate_type.value}: {rate_data.raw_value}% ({rate_data.answer} scaled)"
            )
            ...
            if success:
                logger.info(
                    f"✓ {rate_data.rate_type.value} updated - Block: {receipt['blockNumber']}"
                )
            else:
    ...
    async def sync_all_rates(self, force: bool = False) -> UpdateResult:
        ...
        async with BCBClient() as bcb:
            ...
            logger.info(
                f"Batch updating {len(to_update)} rates: {[r.rate_type.value for r in to_update]}"
            )
            ...
            try:
                ...
                gas_estimate = await tx_data.estimate_gas(
                    {"from": self.account.address}
                )
                ...
                rate_updated_events = [
                    log for log in receipt["logs"] if len(log["topics"]) > 0
                ]
                ...
                if success:
                    logger.info(
                        f"✓ Batch complete - {len(to_update)} updated, {len(to_skip)} skipped - Block: {receipt['blockNumber']}"
                    )
                else:
...
async def main():
    ...
    parser.add_argument(
        "command",
        choices=["sync", "sync-all", "status", "balance", "check"],
        help="Command to run",
    )
    parser.add_argument(
        "--rate", "-r", type=str, help="Rate type for sync/check command"
    )
    parser.add_argument(
        "--force", "-f", action="store_true", help="Force update even if same date"
    )
    ...
    if args.command == "balance":
        ...
        for name, data in rates.items():
            print(
                f"{name:<6} | {data['answer']:<18} | {data['value_percent']:>9.4f}% | {data['real_world_date']}"
            )
        ...
        try:
            ...
            print(
                f"Error: Invalid rate type. Choose from: {[r.value for r in RateType]}"
            )
            return
